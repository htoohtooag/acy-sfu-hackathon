...
- Production CORS now requires `FRONTEND_ORIGIN` when the backend runs with `NODE_ENV=production`, and applies that allow list to both Express and Socket.IO. Next, deploy the backend to Render, then set the real Vercel URL as `FRONTEND_ORIGIN` before testing the full MVP flow.
- The frontend `prebuild` hook now compiles the local `shared` workspace before `next build`, and `frontend/package.json` now declares `shared` as its workspace dependency, so Vercel can resolve `shared/schemas`. Keep Vercel on the Next.js preset with `frontend` as its root directory and allow source files outside that root when the setting is available.
