...
- The frontend `prebuild` hook now compiles the local `shared` workspace before `next build`, so Vercel can resolve `shared/schemas`. Keep Vercel on the Next.js preset with `frontend` as its root directory and allow source files outside that root when the setting is available.
