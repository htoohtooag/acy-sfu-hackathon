...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="flex h-screen min-h-0 overflow-hidden bg-background"><div className="hidden h-full min-h-0 lg:flex"><AppSidebar user={user} recentWorkrooms={recentWorkrooms} /></div><Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}><SheetContent className="left-0 right-auto max-w-sm data-ending-style:-translate-x-full data-starting-style:-translate-x-full"><SheetTitle className="sr-only">Workspace navigation</SheetTitle><AppSidebar user={user} recentWorkrooms={recentWorkrooms} mobile /></SheetContent></Sheet><div className="flex h-full min-h-0 min-w-0 flex-1 flex-col"><AppMobileHeader /><main className="min-h-0 flex-1 overflow-y-auto">{children}</main></div></div>;
}
