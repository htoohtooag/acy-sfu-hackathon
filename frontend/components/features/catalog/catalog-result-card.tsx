    ...
    <article className="group relative overflow-hidden border-b border-border pb-6 transition-colors hover:border-primary/50">
      <div className="grid items-stretch gap-0 md:grid-cols-[minmax(13rem,28%)_1fr]">
        <Link href={`/freelancers/${item.id}`} aria-label={`Open package details for ${item.title}`} className={cn("relative block min-h-56 overflow-hidden bg-gradient-to-br focus-visible:outline-2 focus-visible:outline-offset-[-4px] focus-visible:outline-ring md:min-h-64", visualToneClasses[presentation?.visualTone ?? "primary"])}>
          {presentation?.imageUrl ? <Image src={presentation.imageUrl} alt={`Sample work from ${freelancerName}`} fill sizes="(min-width: 768px) 28vw, 100vw" className="object-cover transition-transform duration-500 group-hover:scale-105" priority={index < 2} /> : null}
        ...
        <div className="flex flex-col gap-5 px-0 py-5 md:px-6 md:py-2">
          ...
          <dl className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-1.5"><Star aria-hidden="true" className="size-4 fill-current text-primary" /><dt className="sr-only">Rating</dt><dd>{presentation?.rating.toFixed(1)} <span className="text-muted-foreground/70">({presentation?.reviewCount})</span></dd></div>
            ...
            <div className="flex items-center gap-1.5"><RotateCcw aria-hidden="true" className="size-4" /><dt className="sr-only">Revisions</dt><dd>{presentation?.revisions}</dd></div>
          </dl>
