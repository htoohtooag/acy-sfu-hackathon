...
export type CatalogPackageDetailPath = "/freelancers" | "/packages";

type CatalogResultCardProps = { item: CatalogPackage; index: number; packageDetailPath?: CatalogPackageDetailPath };
...
export function CatalogResultCard({ item, index, packageDetailPath = "/freelancers" }: CatalogResultCardProps) {
  ...
  const packageHref = `${packageDetailPath}/${item.id}`;
  const sampleWork = item.freelancer.sample_works[0];
  const imageUrl = sampleWork?.image_url || presentation?.imageUrl;
  const sampleWorkProjects: AnimatedFolderProject[] = item.freelancer.sample_works
    .filter((work) => Boolean(work.image_url))
    .slice(0, 3)
    .map((work) => ({ id: work.id, image: work.image_url, title: work.title }));
  ...
  return (
    <article className="group relative overflow-hidden border-b border-border pb-6 transition-colors hover:border-primary/50">
      <div className="grid items-stretch gap-0 md:grid-cols-[minmax(13rem,28%)_1fr]">
        <div className="flex min-w-0 justify-center md:min-h-64 relative">
          {sampleWorkProjects.length > 0 ?
            <AnimatedFolder title="Sample work" projects={sampleWorkProjects} className="max-w-sm" />
            :
            // <Link href={packageHref} aria-label={`Open package details for ${item.title}`} className={cn("relative block min-h-56 w-full overflow-hidden bg-gradient-to-br focus-visible:outline-2 focus-visible:outline-offset-[-4px] focus-visible:outline-ring md:min-h-64", visualToneClasses[presentation?.visualTone ?? "primary"])}>
            //   {imageUrl ?
            //     <Image src={imageUrl} alt={sampleWork?.title ?? `Sample work from ${freelancerName}`} fill sizes="(min-width: 768px) 28vw, 100vw" className="object-cover transition-transform duration-500 group-hover:scale-105" priority={index < 2} /> : <div className="absolute inset-0 menu-pattern" aria-hidden="true" />}
            ...
            // </Link>
            <Image src="/emptystate/sampleworkempty.jpeg" alt={sampleWork?.title ?? `Sample work from ${freelancerName}`} fill sizes="(min-width: 768px) 28vw, 100vw" className="object-cover transition-transform duration-500 group-hover:scale-105" priority={index < 2} />
        ...
        <div className="flex flex-col gap-5 px-0 py-5 md:px-6 md:py-2">
          ...
          <div>
            <Link href={packageHref} className="font-heading text-xl font-semibold leading-tight text-foreground transition-colors hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">{item.title}</Link>
          ...
          <div className="mt-auto flex flex-wrap items-end justify-between gap-4 border-t border-border pt-4">
            ...
            <Link href={packageHref} className="text-sm font-semibold text-foreground underline-offset-4 hover:text-primary hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">View package</Link>
          </div>
        </div>
      </div>
    </article>
