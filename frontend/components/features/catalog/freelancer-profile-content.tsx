...
export function FreelancerProfileContent({ profile, packages, mode, packagePresentation }: FreelancerProfileContentProps) {
  const freelancer = packages[0]?.freelancer;
  const name = freelancer?.user.full_name ?? "TalentScout freelancer";
  const headline = freelancer?.headline ?? "Independent professional";
  const initials = name.split(" ").map((part) => part[0]).join("").slice(0, 2);
  const gallery = profile.portfolioGallery ?? packagePresentation?.gallery ?? [];
  ...
  return <div className="w-full"><div className="mx-auto max-w-7xl space-y-6"><section><FreelancerProfileHero profile={profile} freelancer={freelancer} /><FreelancerProfileMetrics profile={profile} /></section><div className="grid items-start gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]"><FreelancerPortfolio profile={profile} gallery={gallery} name={name} />{packagePresentation ? <FreelancerPackageSidebar item={packages[0]} presentation={packagePresentation} profile={profile} /> : null}</div><FreelancerWorkHistory history={history} /></div><PublicMarketplaceFooter /></div>;
}
