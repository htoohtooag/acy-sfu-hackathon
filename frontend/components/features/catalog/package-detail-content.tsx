...
//           <Button nativeButton={false} size="lg" render={<Link href={`/orders/checkout?packageId=${encodeURIComponent(item.id)}`} />}>
//             Hire this package
//           </Button>
//           {mode === "page" ? <>
//             <Button type="button" size="lg" variant="outline">Start a conversation</Button><Link href="/freelancers" className="text-sm font-medium text-muted-foreground underline-offset-4 hover:text-primary hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">Back to all services</Link></> : null}</div>
//       </section>
...
export function PackageDetailContent({
  ...
  const presentation =
    findCatalogPackageDetailPresentation(item.id) ??
    createFallbackCatalogPackageDetailPresentation(item);

  const fallbackPresentation = catalogPackagePresentation[item.id];

  const freelancerName =
    item.freelancer.user.full_name ?? "Gigmatch freelancer";
  ...
  const relatedPackages = presentation.relatedPackageIds
    .map(
      (id) =>
        mockCatalogPackages.find(
          (packageItem) => packageItem.id === id,
        ) ?? undefined,
    )
    .filter(
      (packageItem): packageItem is CatalogPackage =>
        Boolean(packageItem),
    );

  const otherPackages = getFreelancerPackages(item.freelancer_id).filter(
    (packageItem) => packageItem.id !== item.id,
  );

  const related =
    relatedPackages.length > 0 ? relatedPackages : otherPackages;
    ...
    <dl className="grid grid-cols-2 gap-x-4 gap-y-2 border-t border-border pt-4 text-xs">
      <div>
        ...
        <dd className="mt-1 flex items-center gap-1.5 font-medium text-foreground">
          ...
          />
          <time dateTime={presentation.publishedOn}>
            {new Date(
              `${presentation.publishedOn}T00:00:00Z`,
      ...
      <div>
        ...
        <dd className="mt-1 flex items-center gap-1.5 font-medium text-foreground">
          <MapPin
            ...
            className="size-3.5 text-muted-foreground"
          />
          {item.freelancer.location_city ?? "Myanmar"}
        </dd>
      </div>
      ...
      <div>
        ...
        <dd className="mt-1 flex items-center gap-1.5 font-medium text-foreground">
          ...
          {fallbackPresentation
            ? `${presentation.rating.toFixed(1)} (${presentation.reviewCount})`
            : "New profile"}
        </dd>
      </div>
    </dl>
  ...
  const identity = (
    <Link
      href={`/freelancers/profile/${item.freelancer.id}`}
      className="mt-4 inline-flex items-center gap-2 rounded-xl text-start focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring"
    >
      <span
      ...
      <span>
        ...
        <span className="mt-0.5 block text-xs text-muted-foreground">
          {item.freelancer.headline ?? "Independent professional"}
        </span>
      </span>
    </Link>
  );
  ...
  return (
    <article
      className={cn(
        "mx-auto w-full",
        mode === "page" && "max-w-5xl",
        mode === "modal" && "max-h-[85vh]",
      )}
    >
      <div
        className={cn(
          "grid overflow-hidden rounded-3xl border border-border bg-card shadow-xl",
      ...
      >
        ...
        <section
          className={cn(
            ...
            mode === "modal"
              ? "max-h-[85vh] overflow-y-auto p-5 sm:p-7"
              : "p-5 sm:p-7",
            mode === "modal" ? "gap-3" : "gap-5",
          )}
          aria-labelledby={
            mode === "modal" ? undefined : "package-detail-title"
          }
        >
          <div>
            ...
            {mode === "page" ? (
              <h1
                ...
                className="mt-3 font-heading text-3xl font-semibold leading-tight tracking-tight text-foreground sm:text-4xl"
              >
                {item.title}
              </h1>
            ) : null}
          ...
          {mode === "modal" ? (
            ...
            <section aria-labelledby="package-description-heading">
              <h2
                id="package-description-heading"
          ...
          <section aria-labelledby="package-skills-heading">
            <h2
              id="package-skills-heading"
          ...
          <div
            className={cn(
              ...
              mode === "modal" &&
                "sticky bottom-0 -mx-5 -mb-5 border-t border-border bg-card/95 px-5 py-4 backdrop-blur sm:-mx-7 sm:-mb-7 sm:px-7",
            )}
          >
            <Button
              nativeButton={false}
              ...
              render={
                <Link
                  href={`/orders/checkout?packageId=${encodeURIComponent(
                    item.id,
                  )}`}
                />
              }
            ...
            {mode === "page" ? (
              <>
                ...
                <Link
                  href="/freelancers"
                  className="text-sm font-medium text-muted-foreground underline-offset-4 hover:text-primary hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring"
                >
        ...
        {/* RIGHT SIDE */}
        <section
          className="min-w-0 border-t border-border p-4 sm:p-6 lg:border-s lg:border-t-0"
          aria-label="Package sample work and related work"
        >
