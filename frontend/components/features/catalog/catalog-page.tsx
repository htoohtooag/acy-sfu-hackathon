...
import type { CatalogPackageDetailPath } from "@/components/features/catalog/catalog-result-card";
...
type CatalogPageProps = { filters: CatalogFilterValues; items: CatalogPackage[]; total: number; packageDetailPath?: CatalogPackageDetailPath };
...
export function CatalogPage({ filters, items, total, packageDetailPath = "/freelancers" }: CatalogPageProps) {
  return (
    <main id="main-content" className="flex-1 bg-background">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-14">
        ...
        <div className="grid gap-8 lg:grid-cols-[15rem_minmax(0,1fr)] xl:grid-cols-[17rem_minmax(0,1fr)]">
          <aside className="hidden h-fit max-h-[calc(100vh-7rem)] self-start overflow-y-auto rounded-2xl bg-secondary p-5 shadow-sm lg:sticky lg:top-24 lg:block"><CatalogFilters filters={filters} /></aside>
          <Suspense fallback={<ResultsLoading />}><CatalogResults items={items} filters={filters} total={total} packageDetailPath={packageDetailPath} /></Suspense>
        </div>
      </div>
    </main>
  );
