...
type JobCatalogProps = { result: CatalogJobPostListResponse; search: string; maxBudget: string };

function pageHref(page: number, search: string, maxBudget: string): string {
...
export function JobCatalog({ result, search, maxBudget }: JobCatalogProps) {
  ...
  return (
    <main id="main-content" className="flex-1 bg-muted/20">
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
        ...
        <form action="/jobs" className="mb-8 grid gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm sm:grid-cols-[minmax(0,1fr)_14rem_auto] sm:items-end sm:p-5">
        ...
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground"><strong className="font-semibold text-foreground">{result.total}</strong> open {result.total === 1 ? "project" : "projects"}</p>{search || maxBudget ? <Link href="/jobs" className="text-sm font-semibold text-primary underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">Clear filters</Link> : null}</div>
        {result.items.length > 0 ? <ul className="grid gap-4" aria-label="Open projects">{result.items.map((job) => <li key={job.id}><JobCard job={job} /></li>)}</ul> : <section className="rounded-2xl border border-dashed border-border bg-card px-6 py-16 text-center"><h2 className="font-heading text-2xl font-semibold text-foreground">No projects match those filters.</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-muted-foreground">Try a broader search or clear the budget limit to see more opportunities.</p><Link href="/jobs" className="mt-6 inline-flex min-h-11 items-center rounded-lg bg-primary px-5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">Browse all projects</Link></section>}
        {totalPages > 1 ? <nav aria-label="Job catalog pagination" className="mt-8 flex items-center justify-between gap-4 border-t border-border pt-5"><div>{result.page > 1 ? <Link href={pageHref(result.page - 1, search, maxBudget)} className="inline-flex min-h-11 items-center rounded-lg border border-border bg-card px-4 text-sm font-semibold hover:border-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">Previous</Link> : <span />}</div><p className="text-sm text-muted-foreground">Page {result.page} of {totalPages}</p><div>{result.page < totalPages ? <Link href={pageHref(result.page + 1, search, maxBudget)} className="inline-flex min-h-11 items-center rounded-lg border border-border bg-card px-4 text-sm font-semibold hover:border-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">Next</Link> : <span />}</div></nav> : null}
      </div>
    </main>
  );
}
